            ...
            for (Magus p2 : people) {
                ...
                if (p1 != p2 && p1.getRelationshipWith(p2) == Relationship.NONE) {
                    ...
                    if (Dice.stressDieResult() + modifier + friendMod > 11) {
                        Relationship newRelationShip = p1.getRelationshipWith(p2) == Relationship.ENEMY ? Relationship.NONE : Relationship.FRIEND;
                        p1.setRelationship(p2, newRelationShip);
                        p2.setRelationship(p1, newRelationShip);
                        // also exchange Longevity Ritual Services
                        if (newRelationShip == Relationship.FRIEND) {
						...
						Relationship newRelationShip = p1.getRelationshipWith(p2) == Relationship.FRIEND ? Relationship.NONE : Relationship.ENEMY;
                        p1.setRelationship(p2, newRelationShip);
                        p2.setRelationship(p1, newRelationShip);
                    }
                }
